...
Agent Codemode provides progressive tool discovery to efficiently find relevant tools from the programmatic tools generated from [MCP Servers](../mcp-servers/index.mdx) and [Skills](../skills/index.mdx).

## Progressive Tools Discovery

Progressive Tools Discovery means the agent does not load every tool definition upfront. Instead, it starts with a lightweight query (like `search_tools` or `list_tool_names`), then requests full schemas only for the small set of tools that are relevant to the current task. This reduces token usage, speeds startup, and keeps the model focused on a short list of candidates.
...
from generated.mcp.bash import run_command

# If skills are wired into Codemode, skill bindings are also available:
...
result = await run_skill({"skill_name": "my-skill", "script_name": "run.py"})
```
...
└── mcp/
    ...
    ├── bash.py
    ├── web.py
    └── skills/              # Present when skills are wired into Codemode
        ├── __init__.py
        ├── list_skills.py
        ├── load_skill.py
...
Skill bindings follow the same pattern but route through the `skills__` prefix:

```python
# generated/mcp/skills/run_skill.py
async def run_skill(arguments: dict) -> str:
    """Run a skill script by name."""
    return await _call_tool("skills", "run_skill", arguments)
```

See [Skills — Generated Skill Bindings](../skills/index.mdx#generated-skill-bindings-approach-d) for details on how skill bindings are generated and routed.
